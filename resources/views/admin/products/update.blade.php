        ...
        <form action="{{route('updateProduct',$product->id)}}" enctype="multipart/form-data" method="POST">
            ...
            <div class="col d-flex">
                <div class="col-md-8">
                    ...
                    <div class="col-md-12 d-flex p-0">
                        <div class="col-md-6 p-0 pr-2">
                            ...
                            <select class="form-control mb-2" name="category_id">
                                ...
                                <option value="{{$items->id}}" {{$product->category_id == $items->id ?  'selected' :''}}>{{$items->name}}</option>
                                @error('category_id')
                                <span style="font-size: 14px;color: red;font-weight: 400">{{$message}}</span>
                                @enderror
                                @endforeach
                    ...
                    <div class="form-floating mb-3">
                        ...
                        <textarea class="form-control" name="description" placeholder="nhập mô tả sản phẩm" id="floatingTextarea2" style="height: 100px">{{$product->description}}</textarea>
                        @error('description')
                        <span style="font-size: 14px;color: red;font-weight: 400">{{$message}}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3 flex-nowrap">
                        ...
                        <input type="file" class="form-control" value="{{$product->image}}" name="file_upload">
                        @error('image')
                        <span style="font-size: 14px;color: red;font-weight: 400">{{$message}}</span>
                        @enderror
                    </div>
            ...
            <div class="col-md-4">
                <div class="mb-3">
                    ...
                    <input type="number" name="sale" value="{{$product->sale}}" class="form-control" placeholder="nhập khuyến mãi">
                    @error('sale')
                    <span style="font-size: 14px;color: red;font-weight: 400">{{$message}}</span>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" value="{{$product->price}}" name="price" placeholder="nhập giá">
                    @error('price')
                    <span style="font-size: 14px; color: red;font-weight: 400">{{$message}}</span>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" value="{{$product->quantity}}" value="1" name="quantity">
                </div>
