   <div class="card">
       <div class="d-flex justify-content-between w-100">
           ...
           <button class="btn btn-primary waves-effect waves-light m-4" data-bs-toggle="modal"
               data-bs-target="#kt_modal_add_product"><i class="ti ti-plus mr-3"></i> Tạo mới</button>
       </div>
       ...
       <div class="table-responsive text-nowrap p-4">
               ...
               <tbody class="table-border-bottom-0 ">
                   @if (count($list_product) > 0)
                       @foreach ($list_product as $i => $item)
                           <tr>
                               <td>{{ ++$i }}</td>
                               <td style="max-width:300px; white-space: initial;"> {{ $item->name }}</td>
                               <td>
                                   <img src="{{ asset('storage/uploads/' . $item->image) }}" alt=""
                                       width="100%" height="50px" style="object-fit: contain">
                               </td>
           ...
           <div class="mt-3"> {!! $list_product->links() !!}</div>
       </div>
   </div>
